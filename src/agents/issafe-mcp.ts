...
export class IsSafeMcp extends McpAgent<Env> {
  ...
  async init() {
    this.server.registerTool(
      ...
      async ({ url }) => {
        try {
          const started = await startStoreCheck({ url }, this.env)
          const report = await waitForReport(started.report.id, this.env, started.report)
    ...
    this.server.registerTool(
      ...
      async ({ id }) => {
        try {
          const result = await getStoreCheck(id, this.env)
